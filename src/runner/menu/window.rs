...
fn fill_rect(frame: &mut [u8], frame_width: u32, x: u32, y: u32, w: u32, h: u32, color: [u8; 4]) {
...
///
/// 在标尺区域顶部渲染可见的小节号数字，与 UI 标尺对齐。
fn composite_ruler_numbers(
    frame: &mut [u8],
    frame_width: u32,
    frame_height: u32,
    scroll_x: f32,
    ...
    // 文本颜色：白色（BGRA 格式）
    let text_color: [u8; 4] = [220, 220, 220, 255]; // 浅灰 B,G,R,A
