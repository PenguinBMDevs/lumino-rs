...
impl EditorData {
    ...
    ///
    /// 返回 `(参照 tick, 参照 key, tick 偏移, key 偏移, 音轨索引)` 列表（按值引用，
    ...
    ///
    /// 返回 `(tick, key, length, velocity, channel, 音轨索引, is_added)` 列表（按值），
    ...
    ///
    /// 返回 `(is_add, tick, key, length, velocity, channel, 音轨索引)` 列表（按值），
