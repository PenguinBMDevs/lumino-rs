...
fn test_dragging_selection_commit_only_modifies_selected() {
    ...
    assert_eq!(
        data.get_note_view(4).expect("第 5 个音符视图应存在").tick,
        450.0
    );
    assert_eq!(
        data.get_note_view(4).expect("第 5 个音符视图应存在").key,
        65
    );
    // 未选中：1,3 不变
    assert_eq!(
    ...
    assert_eq!(
        data.get_note_view(3).expect("第 4 个音符视图应存在").tick,
        300.0
    );
    assert_eq!(
        data.get_note_view(3).expect("第 4 个音符视图应存在").key,
        63
    );
}
