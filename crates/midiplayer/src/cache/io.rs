//! 缓存文件的磁盘读写操作
//!
//! 提供 `write_track_tile_cache` / `read_track_tile_cache` 及其内部辅助函数。
//! 文件格式定义见父模块文档。
...
use super::core::{CacheError, CacheMeta, MAGIC, VERSION, ZSTD_LEVEL};
...
pub fn write_track_tile_cache(
    ...
    let path = super::core::cache_path(cache_dir, midi_hash, tile.track_idx, tile.time_group);
...
pub fn read_track_tile_cache(
    ...
    let path = super::core::cache_path(cache_dir, midi_hash, track_idx, time_group);
    if !path.exists() {
        return Ok(None);
    }
