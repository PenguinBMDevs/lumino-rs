...
fn test_single_note_drag_with_clamp_undo_restores_original() {
    ...
    assert!(editor.commit_current_edit());
    assert_eq!(
        editor.editor_state.data.get_note_view(0).unwrap().key,
        127,
        "应 clamp 到 127"
    );
    ...
    assert!(editor.undo());
    assert_eq!(editor.editor_state.data.get_note_view(0).unwrap().key, 100);
    assert_eq!(
        editor.editor_state.data.get_note_view(0).unwrap().tick,
        50.0
    );
...
fn test_batch_drag_commit_undo_redo_flow() {
    ...
    assert_eq!(data.get_note_view(1).unwrap().tick, 240.0); // 未选中，不变
    ...
    assert_eq!(data.get_note_view(2).unwrap().tick, 480.0);
...
fn test_multiple_drags_create_independent_undo_steps() {
    ...
    assert_eq!(editor.editor_state.data.get_note_view(0).unwrap().key, 68);
