...
fn test_single_note_drag_with_clamp_undo_restores_original() {
    ...
    editor
        ...
        .data
        .notes
        .push_back(Note::new(50.0, 100, 480.0));
    ...
    assert!(editor.commit_current_edit());
    assert_eq!(
        editor.editor_state.data.notes[0].key, 127,
        "应 clamp 到 127"
    );

    // 撤销应恢复到 key=100（原值），而不是 clamp 前的 200
    assert!(editor.undo());
    assert_eq!(editor.editor_state.data.notes[0].key, 100);
...
fn test_batch_drag_commit_undo_redo_flow() {
    ...
    editor
        ...
        .data
        .notes
        .push_back(Note::new(0.0, 60, 240.0));
    editor
        .editor_state
        .data
        .notes
    ...
    assert_eq!(notes[1].tick, 240.0); // 未选中，不变
    ...
    assert_eq!(notes[2].tick, 480.0);
    assert_eq!(notes[2].key, 64);
...
fn test_multiple_drags_create_independent_undo_steps() {
    ...
    assert_eq!(editor.editor_state.data.notes[0].key, 68);
...
fn test_cross_group_logical_undo_after_split() {
    ...
    // 现在 notes 有 4 个音符
    assert_eq!(editor.editor_state.data.notes.len(), 4);
    ...
    assert_eq!(editor.editor_state.data.notes.len(), 3);
    ...
    assert_eq!(
        editor.editor_state.data.notes.len(),
        0,
