...
fn test_commit_pending_copy_inserts_copies_and_selects_them() {
    ...
    assert_eq!(data.get_note_view(0).unwrap().key, 60, "原始音符不变");
    assert_eq!(data.get_note_view(1).unwrap().tick, 200.0);
    assert_eq!(data.get_note_view(1).unwrap().key, 67);
    assert_eq!(
        data.get_note_view(1).unwrap().length,
        480.0,
        "长度复制自原始"
    );
    assert_eq!(data.get_note_view(2).unwrap().tick, 240.0);
    assert_eq!(data.get_note_view(2).unwrap().key, 62, "未选中音符不变");
...
fn test_commit_pending_copy_multiple_notes() {
    ...
    assert_eq!(data.get_note_view(1).unwrap().key, 63);
    assert_eq!(data.get_note_view(2).unwrap().tick, 240.0);
    assert_eq!(data.get_note_view(3).unwrap().tick, 340.0);
    assert_eq!(data.get_note_view(3).unwrap().key, 65);
    assert_eq!(data.get_note_view(4).unwrap().tick, 480.0);
    assert_eq!(data.get_note_view(5).unwrap().tick, 580.0);
    assert_eq!(data.get_note_view(5).unwrap().key, 67);
...
fn test_commit_current_edit_commits_pending_copy() {
    ...
    assert_eq!(
        editor.editor_state.data.get_note_view(1).unwrap().tick,
        100.0
    );
    assert!(editor.pending_copy_drag_state.is_none());
}
...
fn test_move_and_copy_both_pending_commit_in_correct_order() {
    ...
    assert_eq!(data.get_note_view(0).unwrap().tick, 100.0, "A 应被移动");
    assert_eq!(data.get_note_view(0).unwrap().key, 60);
    assert_eq!(data.get_note_view(1).unwrap().tick, 240.0, "B 不变");
    assert_eq!(data.get_note_view(2).unwrap().tick, 290.0, "B 副本已写入");
