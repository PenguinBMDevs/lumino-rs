...
pub fn build_cc_bar_instances(
    ...
    cc_points: &[lumino_core::CcPoint],
    bend_points: &[lumino_core::BendPoint],
    ...
    let panel_x = canvas_offset_x;
    let panel_y = canvas_offset_y + canvas_size_y;
    ...
    if is_velocity {
        ...
        for point in velocity_points {
            ...
            instances.push(CcBarInstance::new(
                bar_x, bar_y, bar_w, bar_h, bar_color,
            ));
        }
    } else if is_bend {
        ...
        for point in bend_points {
            ...
            let bar_h = normalized * graph_height;
            let bar_x = panel_x + keyboard_width + point.tick * zoom_x - scroll_x;
            let bar_y = actual_panel_y + TOOLBAR_HEIGHT + max_y - bar_h;
