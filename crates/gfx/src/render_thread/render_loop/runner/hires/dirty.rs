...
use crate::{
    HiResConfig, HiResRenderer, TileCoord,
...
use super::common::{push_onion_progress, ensure_renderer_for_config};

/// 对所有目标 time_group 生成并上传脏区域覆层贴图
fn generate_and_upload_dirty_overlays(
    renderer: &mut HiResRenderer, ctx: &RenderContext,
    sorted_notes: &[Vec<OnionSkinNote>], target_time_groups: &[u32],
    ticks_per_group: u32, width: u32, key_count: u16,
    track_group: u32, track_start: u16, track_end: u16, track_count: u16,
) {
    for &time_g in target_time_groups {
        ...
        let group_tile = crate::merge_group_tiles(
            &track_tiles, merged_coord, tick_start, tick_end,
            width, key_count, (track_start, track_end),
        );

        renderer.upload_dirty_overlay(
            &ctx.device, &ctx.queue, merged_coord,
            &group_tile.pixels, group_tile.width, group_tile.height,
        );
    }
}
...
pub(crate) fn handle_show_dirty_overlay(
    ...
    let HiResTrackParams { track_idx, group_notes, dirty_time_groups, ppq, key_count, total_ticks, track_count, config, midi_hash: _ } = params;
    ...
    if hires_meta.is_none() {
        *hires_meta = Some(HiResMeta {
            track_count: needed_track_count, track_groups: 1, key_count,
    ...
    for notes in &mut sorted_notes { notes.sort_by(|a, b| a.start_ms.total_cmp(&b.start_ms)); }
    ...
    if let Some(renderer) = hires_renderer {
        generate_and_upload_dirty_overlays(
            renderer, ctx, &sorted_notes, &target_time_groups,
            ticks_per_group, width, key_count,
            track_group, track_start, track_end, track_count,
        );
    }

    push_onion_progress(onion_progress, &format!("音轨组 {track_group} 脏区域临时覆层已生成"), 1.0);
}
