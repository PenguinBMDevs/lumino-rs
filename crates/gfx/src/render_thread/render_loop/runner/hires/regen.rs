...
use crate::{
    HiResConfig, HiResRenderer,
    TRACKS_PER_GROUP, generate_track_tile,
};
use crate::render_thread::HiResTrackParams;
use lumino_onion_skin_hires::{
    CacheMeta, merge_track_tile_into, read_track_tile_cache,
};
...
fn regen_time_group_merged_pixels(
    time_g: u32, ticks_per_group: u32, track_group: u32, all_track_groups: u32,
    sorted_notes: &[Vec<OnionSkinNote>], track_count: u16, track_start: u16,
    config: &HiResConfig, mh: &str, width: u32, key_count: u16,
    ppq: u16, measures_per_group: u32, cache_dir: &std::path::Path,
) -> Vec<u8> {
        ...
        if tg == track_group {
            ...
            for t in tg_start..tg_end {
                let expected_meta = CacheMeta {
                    track_idx: t, time_group: time_g, width,
                    height: key_count as u32, tick_start, tick_end,
                    key_count, ppq, measures_per_group,
                };
                if let Ok(Some(tile)) =
...
pub(crate) fn handle_regenerate_hires_track(
    params: HiResTrackParams, ctx: &RenderContext,
    ...
    let HiResTrackParams { track_idx, group_notes, dirty_time_groups: _, ppq, key_count, total_ticks, track_count, config, midi_hash } = params;
    ...
    if hires_meta.is_none() {
        *hires_meta = Some(HiResMeta {
            track_count, track_groups: 1, key_count,
            time_groups: config.time_group_count(total_ticks, ppq),
            ticks_per_group: config.ticks_per_group(ppq),
        });
    }

    let (width, track_start) = (config.tile_width_px, (track_group * TRACKS_PER_GROUP as u32) as u16);
    let (ticks_per_group, time_groups) = (config.ticks_per_group(ppq), config.time_group_count(total_ticks, ppq));
    let (all_track_groups, measures_per_group) = (config.track_group_count(track_count), config.measures_per_group);
    ...
    std::thread::spawn(move || {
        ...
        for n in &mut notes { n.sort_by(|a, b| a.start_ms.total_cmp(&b.start_ms)); }

        for time_g in 0..time_groups {
            let merged = regen_time_group_merged_pixels(
                time_g, ticks_per_group, track_group, all_track_groups,
                &notes, track_count, track_start,
                &config, &mh, width, key_count, ppq, measures_per_group, &cache_dir,
            );
            if let Ok(guard) = tx.lock() {
                let _ = guard.send(HiResStreamMsg::TimeGroupMerged {
                    track_group, time_group: time_g, pixels: merged,
                    width, height: key_count as u32,
                });
            }
        }

        if let Ok(guard) = tx.lock() { let _ = guard.send(HiResStreamMsg::ClearDirtyOverlay(track_group)); }
        if let Ok(guard) = tx.lock() { let _ = guard.send(HiResStreamMsg::Finished); }
    });
}
