...
use crate::{GroupTile, HiResProgressCallback, HiResRenderer, HiResConfig};
...
fn setup_generate_context(context: &mut HiResGenerateContext<'_>) {
    ensure_renderer_for_config(
        context.ctx, context.hires_renderer, context.hires_config, &context.config,
    );

    let track_count = context.notes.len() as u16;
    let time_groups = context.config.time_group_count(context.total_ticks, context.ppq);
    let ticks_per_group = context.config.ticks_per_group(context.ppq);
    *context.hires_meta = Some(HiResMeta {
        track_count, track_groups: 1, key_count: context.key_count,
        time_groups, ticks_per_group,
    });
}
...
fn spawn_streaming_generation(
    ...
    config: HiResConfig, midi_hash: String,
    ppq: u16, key_count: u16, total_ticks: u32,
) {
    ...
    std::thread::spawn(move || {
            ...
            move |time_group: u32, tile: GroupTile| {
                if let Ok(guard) = tx.lock() {
                    let _ = guard.send(HiResStreamMsg::TimeGroupMerged {
                        ...
                        time_group, pixels: tile.pixels, width: tw, height: th,
                    });
                }
            }
        };
        ...
        let stream_ctx = StreamingGenContext {
            config: &config, ppq, key_count, total_ticks, midi_hash: &midi_hash,
        };
        lumino_onion_skin_hires::generate_all_tiles_streaming(
            &mut notes, &stream_ctx, Some(cb), &time_group_cb,
        );
...
pub(crate) fn handle_generate_hires(mut context: HiResGenerateContext<'_>) {
    ...
    push_onion_progress(context.onion_progress, "正在后台生成高精度洋葱皮贴图\u{2026}", 0.0);

    spawn_streaming_generation(
        context.onion_progress.clone(), context.hires_result_tx,
        context.config.tile_width_px, context.key_count as u32,
        context.notes, context.config, context.midi_hash,
        context.ppq, context.key_count, context.total_ticks,
    );
}
