...
pub(super) fn handle_hires_control(
    ...
    match cmd {
        ControlCommand::GenerateHiResOnionSkin { notes, ppq, key_count, total_ticks, config, midi_hash } =>
            generate::handle_generate_hires(HiResGenerateContext {
                ctx, notes, ppq, key_count, total_ticks, config, midi_hash,
                hires_result_tx, onion_progress, hires_renderer, hires_meta, hires_config,
            }),
        ...
        ControlCommand::RegenerateHiResTrack(params) =>
            regen::handle_regenerate_hires_track(params, ctx, hires_result_tx, hires_renderer, hires_meta, hires_config),
        ControlCommand::ShowHiResDirtyOverlay(params) =>
            dirty::handle_show_dirty_overlay(params, ctx, hires_renderer, hires_meta, hires_config, onion_progress),
        _ => {}
    }
}
