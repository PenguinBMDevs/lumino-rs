...
mod tests {
    ...
    fn test_encode_decode_midi_data() {
        ...
        let encoded =
            encode_loaded_data(&data, LoadedDataType::Midi).expect("编码已加载的MIDI数据失败");
        let (dtype, decoded_bytes) = decode_loaded_data(&encoded).expect("解码已加载的数据失败");

        assert_eq!(dtype, LoadedDataType::Midi);
        let decoded: LoadedMidiData =
            bincode::deserialize(&decoded_bytes).expect("反序列化LoadedMidiData失败");
