//! 高精度洋葱皮贴图导出
//!
//! 将 `LuminoProject` 的音轨数据转换为 `OnionSkinNote` 并生成 `.lmocache`
...
pub fn export_onion_skin_tiles(
    ...
    // generate_all_tiles 内部已启用后台缓存写入线程，会在生成的同时
...
/// 收集所有已加载或已修改音轨的洋葱皮音符
fn collect_onion_skin_notes(project: &LuminoProject) -> Vec<Vec<OnionSkinNote>> {
    project
        .tracks
        .iter()
        .enumerate()
...
/// 从单轨 `LmtrackData` 提取 `OnionSkinNote` 列表
///
...
fn track_notes_from_lmtrack(
    ...
    for ev in events {
        ...
        {
            notes.push(OnionSkinNote::from_ms(
                start as f32,
                current_tick as f32,
                key as u8,
                color,
