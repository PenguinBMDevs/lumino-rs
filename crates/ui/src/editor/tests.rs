...
mod tests {
    ...
    fn test_tick_to_x_conversion() {
        ...
        let tick = 100.0;
        let expected_x = tick * editor.state.zoom_x + editor.state.keyboard_width - editor.state.scroll_x;
    ...
    fn test_x_to_tick_conversion() {
        let editor = Editor::new();
        let x = 200.0;
        let expected_tick = (x - editor.state.keyboard_width + editor.state.scroll_x) / editor.state.zoom_x;
    ...
    fn test_tick_x_roundtrip() {
        ...
        // 允许浮点误差
        assert!((original_tick - recovered_tick).abs() < 0.01, 
            "Roundtrip failed: original={}, recovered={}", original_tick, recovered_tick);
    }
