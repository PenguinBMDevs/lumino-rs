...
impl super::Editor {
    ...
    pub fn set_visible_key_count(&mut self, count: u16) {
        ...
        // 向上拓展：高键号在上方，扩展键（128-255）应出现在原有键位之�?        // 原有最高键(127)�?world_y �?0 变为 added_keys*zoom_y
        // 需要同步增�?scroll_y 使原有可见区域保持不�?        if count > old_count {
            let added_keys = (count - old_count) as f32;
            self.editor_state.view.scroll_y += added_keys * self.editor_state.view.zoom_y;
            // 重新钳位到有效范�?            let vh = (canvas_height - self.editor_state.view.ruler_height).max(0.0);
        ...
        // 键盘和网格缓存都需要刷新：key_count 变了，键盘绘制和网格线都要重�?        self.invalidate_caches(super::CacheInvalidation::ALL);
    }
